...
$image = imagecreatefrombmp("1.bmp");

//echo "X: ".imagesx($image)." Y: ".imagesy($image);

$xpos = round(imagesx($image) / 2);
$ypos = round(imagesy($image) / 2);

$firstcolor = imagecolorat($image, 0, $ypos);
$tempcolor = null;
$tempcolory = null;
...
for ($x = 1; $x < imagesx($image); $x++) {
    $color = imagecolorat($image, $x, $ypos);
    if ($firstcolor != $color) {
        if ($tempcolor == $color || $tempcolor == null) {
            
            for ($y = 1; $y < imagesy($image); $y++) {
                $colory = imagecolorat($image, $xpos, $y);
